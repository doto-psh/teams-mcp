"""
OAuth Configuration Management for Microsoft Teams

This module centralizes OAuth-related configuration for Microsoft Graph API integration.
Adapted from Google Workspace MCP for Microsoft Teams.
...
class OAuthConfig:
    """
    Centralized OAuth configuration management for Microsoft Teams.

    This class provides a single source of truth for all OAuth-related 
    configuration values for Microsoft Graph API integration.
    """

    def __init__(self):
        # Base server configuration
        self.base_uri = os.getenv("TEAMS_MCP_BASE_URI", "http://localhost")
        self.port = int(os.getenv("PORT", os.getenv("TEAMS_MCP_PORT", "8000")))
        ...
        self.tenant_id = os.getenv("MICROSOFT_TENANT_ID", "common")  # Support multi-tenant
        ...
        # Microsoft-specific OAuth endpoints
    ...
    def is_configured(self) -> bool:
        """
        Check if OAuth is properly configured for Microsoft Teams.
    ...
    def get_microsoft_auth_url(self, scopes: List[str], state: str, code_challenge: Optional[str] = None) -> str:
        """
        ...
        if code_challenge and self.pkce_required:
    ...
    def get_environment_summary(self) -> dict:
        """
        Get a summary of the current OAuth configuration for Microsoft Teams.

"""
Microsoft Teams Authentication Provider for FastMCP

This module implements OAuth 2.1 authentication for Microsoft Teams using FastMCP's
built-in authentication patterns. It acts as a Resource Server (RS) that trusts
Microsoft as the Authorization Server (AS).

Key features:
- JWT token verification using Microsoft's public keys
- Discovery metadata endpoints for MCP protocol compliance
- CORS proxy endpoints to work around Microsoft's CORS limitations
- Session bridging to Microsoft credentials for API access
"""
...
class MicrosoftTeamsAuthProvider(AuthProvider):
    """
    Authentication provider for Microsoft Teams integration.
    
    This provider verifies JWT tokens issued by Microsoft:
    - Microsoft acts as the Authorization Server (AS)
    - This server acts as the Resource Server (RS)
    - Tokens are verified using Microsoft's public keys
    """

    def __init__(self):
        """Initialize the Microsoft Teams auth provider."""
        super().__init__()
        
        # Microsoft OAuth client configuration
        ...
        if not self.client_id:
            logger.warning("MICROSOFT_OAUTH_CLIENT_ID not set - OAuth 2.1 authentication will not work")
            
        if not self.client_secret:
            logger.warning("MICROSOFT_OAUTH_CLIENT_SECRET not set - OAuth 2.1 authentication will not work")
        
        # Initialize JWT verifier for Microsoft tokens
    ...
    async def verify_token(self, token: str) -> Optional[AccessToken]:
        """
        Verify a bearer token issued by Microsoft.
        
        Args:
            token: The bearer token to verify
            
        Returns:
            Verified authentication context or None
        """
        if not self.client_id:
            return None
        ...
        try:
            ...
            if access_token:
                ...
                if user_email:
                    ...
                    session_id = f"microsoft_{access_token.claims.get('sub', 'unknown')}"
                ...
                logger.debug(f"Successfully verified Microsoft token for user: {user_email}")
                
            return access_token
            
        except Exception as e:
            logger.error(f"Failed to verify Microsoft token: {e}")
            return None

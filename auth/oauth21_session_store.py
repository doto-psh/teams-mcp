"""
OAuth 2.1 Session Store for Microsoft Teams

This module provides a global store for OAuth 2.1 authenticated sessions
that can be accessed by Teams service decorators. It also includes
session context management and credential conversion functionality.

Adapted from Microsoft Teams MCP for Microsoft Graph API.
"""
...
class MicrosoftCredentials:
    """Container for Microsoft Graph API credentials."""
    token: str
...
class OAuth21SessionStore:
    ...
    authenticated credentials, allowing Microsoft services to access them.
    ...
    def store_session(
        ...
        Args:
            ...
            issuer: Token issuer (e.g., "https://login.microsoftonline.com/{tenantid}/v2.0")
        """
        with self._lock:
            session_info = {
    ...
    def get_credentials(self, user_email: str) -> Optional[MicrosoftCredentials]:
        """
        Get Microsoft credentials for a user from OAuth 2.1 session.
        
        Args:
            user_email: User's email address
            
        Returns:
            Microsoft Credentials object or None
        """
        with self._lock:
            ...
            try:
                # Create Microsoft credentials from session info
                credentials = MicrosoftCredentials(
                    ...
                    tenant_id=session_info.get("tenant_id", "common"),
                )
    ...
    def get_credentials_by_mcp_session(self, mcp_session_id: str) -> Optional[MicrosoftCredentials]:
        """
        Get Microsoft credentials using FastMCP session ID.
    ...
    def get_credentials_with_validation(
        ...
        """
        Get Microsoft credentials with session validation.
        ...
        Returns:
            Microsoft Credentials object if validation passes, None otherwise
        """
        with self._lock:
            ...
            # Fallback: If user has valid credentials and no conflicting session, allow access
            # This handles cases where OAuth succeeded but session binding failed
            if requested_user_email in self._sessions:
                session_info = self._sessions[requested_user_email]
                if session_info.get("access_token"):
                    logger.info(
                        f"Allowing credential access for {requested_user_email} based on valid OAuth session "
                        f"(fallback mode - session binding may have failed)"
                    )
                    return self.get_credentials(requested_user_email)
...
# =============================================================================
# Microsoft Credentials Bridge (absorbed from oauth21_microsoft_bridge.py)
# =============================================================================
...
def get_credentials_from_token(access_token: str, user_email: Optional[str] = None) -> Optional[MicrosoftCredentials]:
    """
    Convert a bearer token to Microsoft credentials.
    ...
    try:
        ...
        # Assume token is valid for 1 hour (typical for Microsoft tokens)
        ...
        credentials = MicrosoftCredentials(
            ...
            client_id=_auth_provider.client_id if _auth_provider else None,
            client_secret=_auth_provider.client_secret if _auth_provider else None,
            scopes=None,  # Will be populated from token claims if available
            expiry=expiry,
            tenant_id="common"
        )
        
        logger.debug("Created Microsoft credentials from bearer token")
        return credentials
        
    except Exception as e:
        logger.error(f"Failed to create Microsoft credentials from token: {e}")
        return None
...
def store_token_session(token_response: dict, user_email: str, mcp_session_id: Optional[str] = None) -> str:
    ...
    Args:
        token_response: OAuth token response from Microsoft
        user_email: User's email address
    ...
    try:
        ...
        session_id = f"microsoft_{user_email}"
        store.store_session(
            ...
            session_id=session_id,
            mcp_session_id=mcp_session_id,
            issuer="https://login.microsoftonline.com",  # Add issuer for Microsoft tokens
        )

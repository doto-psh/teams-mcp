...
from auth.teams_auth import handle_auth_callback, check_client_secrets
...
class MinimalOAuthServer:
    ...
    def _setup_callback_route(self):
        ...
        async def oauth_callback(request: Request):
            ...
            if error:
                error_message = f"Authentication failed: Microsoft returned an error: {error}. State: {state}."
                logger.error(error_message)
                return create_error_response(error_message)

            if not code:
                error_message = "Authentication failed: No authorization code received from Microsoft."
                logger.error(error_message)
                return create_error_response(error_message)
            ...
            try:
                ...
                # Exchange code for credentials using Microsoft Teams auth
                success, message, user_email = await handle_auth_callback(
                    code=code,
                    state=state or "",
                    session_id=""
                )
                ...
                if success and user_email:
                    logger.info(f"OAuth callback: Successfully authenticated user: {user_email} (state: {state}).")
                    ...
                    return create_error_response(message or "Authentication failed")
